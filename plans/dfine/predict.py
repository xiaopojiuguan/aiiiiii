...
DEIM-D-FINE-X 推理脚本
======================
- 加载 DEIM-D-FINE-X (HGNetv2-X + FDR + DEIM) 训练好的 checkpoint
- FDR 分布回归 → 亚像素精度框坐标
- 1280×1280 ROI 切片推理 + 全局视图
- 坐标还原 → 后处理 → submission.json
- 无需 NMS（DETR 架构天然无重复框）

用法:
  python plans/dfine/predict.py --weights outputs/checkpoints/deim_dfine_x_XXX/best.pth
  python plans/dfine/predict.py --weights best.pth --conf 0.01 --batch 2
"""
...
DEIM_DIR = PROJECT_ROOT / "DEIM"
sys.path.insert(0, str(DEIM_DIR / "src"))
...
logger = logging.getLogger("deim_dfine_pred")


class DEIMDFINEPredictor:
    """DEIM-D-FINE-X 推理器 —— FDR 分布精修 + DEIM 密集匹配训练，无 NMS。"""
    ...
    def __init__(
        ...
        batch_size: int = 2,  # HGNetv2-X 更重，默认小一些
    ):
        ...
        logger.info(f"Loading DEIM-D-FINE-X: {weights_path}")
...
def main():
    parser = argparse.ArgumentParser(description="DEIM-D-FINE-X Inference")
    ...
    output_path = Path(args.output) if args.output else OUTPUT_ROOT / f"submission_dfine_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    print(f"\n[DEIM-D-FINE-X Inference Complete]")
